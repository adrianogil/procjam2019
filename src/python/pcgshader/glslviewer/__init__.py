...
class GlslViewer:
    ...
    def run(self):
        glslviewer_cmd = ' '.join(self.cmd)

        print('EXE : %s' % (glslviewer_cmd,))

        glslviewer_output = subprocess.check_output(glslviewer_cmd, shell=True)
        glslviewer_output = glslviewer_output.strip()
        print(glslviewer_output)
